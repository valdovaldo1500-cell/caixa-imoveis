...
export default function MapaPage() {
  ...
  const fetchData = useCallback(() => {
    ...
    if (modalidades.length > 0) urlParams.set("modalidade", modalidades.join(","));
    if (descontoMin) urlParams.set("desconto_min", descontoMin);
    if (precoMax) urlParams.set("preco_max", precoMax);
    if (tipos.length > 0) urlParams.set("tipo", tipos.join(","));
    if (distancia) urlParams.set("max_distance", distancia);
    urlParams.set("uf", state.toUpperCase());

    const url = `/api/map?${urlParams.toString()}`;
  ...
  }, [modalidades, descontoMin, precoMax, tipos, distancia, state]);
